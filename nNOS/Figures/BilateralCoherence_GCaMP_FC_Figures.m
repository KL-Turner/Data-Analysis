function [] = BilateralCoherence_GCaMP_FC_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
        ...
        for cc = 1:length(dataTypes)
            ...
            for dd = 1:length(behaviors)
                ...
                if isempty(Results_BilatCoher_GCaMP.(group).(animalID).(dataType).(behavior).C) == false
                    ...
                    data.(group).(dataType).(behavior).f = cat(1,data.(group).(dataType).(behavior).f,Results_BilatCoher_GCaMP.(group).(animalID).(dataType).(behavior).ff);
    ...
    for bb = 1:length(behaviors)
        ...
        for cc = 1:length(dataTypes)
            ...
            for dd = 1:length(variables)
                ...
                data.(group).(dataType).(behavior).(['stdErr_' variable]) = std(data.(group).(dataType).(behavior).(variable),0,1)./sqrt(size(data.(group).(dataType).(behavior).(variable),1));
            end
        end
    end
end
...
for aa = 1:length(dataTypes)
    ...
    sgtitle([dataType ' bilateral coherence [GCaMP FC]'])
    for bb = 1:length(behaviors)
        behavior = behaviors{1,bb};
        subplot(2,3,bb);
    ...
    % save figure(s)
    if saveFigs == true
        dirpath = [rootFolder delim 'Summary Figures' delim 'Bilateral Coherence' delim];
        if ~exist(dirpath,'dir')
            mkdir(dirpath);
        end
        savefig(summaryFigure,[dirpath 'BilateralCoherence_GCaMP_FC_' dataType]);
        set(summaryFigure,'PaperPositionMode','auto');
        print('-vector','-dpdf','-fillpage',[dirpath 'BilateralCoherence_GCaMP_FC_' dataType])
    end
end
...
for aa = 1:length(dataTypes)
    dataType = dataTypes{1,aa};
    summaryFigure = figure;
    sgtitle([dataType ' peak coherence [GCaMP FC]'])
    for bb = 1:length(arousalStates)
        ...
        for cc = 1:length(freqs)
            ...
            s1 = scatter(xInds*zz,data.Blank_SAP.(dataType).(arousalState).(freq),75,'MarkerEdgeColor','k','MarkerFaceColor',colors('north texas green'),'jitter','off','jitterAmount',0.25);
            hold on;
            ...
            s2 = scatter(xInds*zz,data.SSP_SAP.(dataType).(arousalState).(freq),75,'MarkerEdgeColor','k','MarkerFaceColor',colors('electric purple'),'jitter','off','jitterAmount',0.25);
    ...
    % save figure(s)
    if saveFigs == true
        savefig(summaryFigure,[dirpath 'BilateralCoherence_GCaMP_FC_' dataType '_FreqPeaks']);
        set(summaryFigure,'PaperPositionMode','auto');
        print('-vector','-dpdf','-fillpage',[dirpath 'BilateralCoherence_GCaMP_FC_' dataType '_FreqPeaks'])
    end
end
...
if saveFigs == true
    % statistical diary
    diaryFile = [dirpath 'BilateralCoherence_GCaMP_FC_Statistics.txt'];
