function [] = NeuralHemoCoherence_FC_GCaMP_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for qq = 1:length(hemispheres)
    ...
    for aa = 1:length(dataTypes)
        ...
        sgtitle([hemisphere ' ' dataType ' neural-hemo coherence [FC GCaMP]'])
        for bb = 1:length(behaviors)
            behavior = behaviors{1,bb};
            subplot(2,3,bb);
        ...
        % save figure(s)
        if saveFigs == true
            dirpath = [rootFolder delim 'Summary Figures' delim 'Neural-Hemo Coherence FC GCaMP' delim];
            if ~exist(dirpath,'dir')
                mkdir(dirpath);
            end
            savefig(summaryFigure,[dirpath 'NeuralHemoCoherence_FC_GCaMP_' hemisphere '_' dataType]);
        end
    end
end
