function [] = WhiskEvoked_2P_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
plot(data.SSP_SAP.contra.mean_timeVector,data.SSP_SAP.contra.mean_diameter - data.SSP_SAP.contra.stdErr_diameter,'color',colors('electric purple'),'LineWidth',0.5)
title('Contralateral whisker stimulation [2P]')
ylabel('\DeltaD/D (%)')
...
% save figure(s)
if saveFigs == true
    dirpath = [rootFolder delim 'Summary Figures' delim 'Stim Evoked 2P' delim];
    if ~exist(dirpath,'dir')
        mkdir(dirpath);
    end
    savefig(summaryFigure,[dirpath 'StimEvoked_2P_Diameter']);
end