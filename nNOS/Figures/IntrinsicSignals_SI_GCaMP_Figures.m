function [] = IntrinsicSignals_SI_GCaMP_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for qq = 1:length(dataTypes)
    ...
    for zz = 1:length(hemispheres)
        ...
        for aa = 1:length(variables)
            ...
            sgtitle(['IOS ' hemisphere ' ' dataType ' ' variable ' [SI GCaMP]'])
            for bb = 1:length(behaviors)
                ...
                % save figure(s)
                if saveFigs == true
                    dirpath = [rootFolder delim 'Summary Figures' delim 'Intrinsic Signals SI GCaMP' delim];
                    if ~exist(dirpath,'dir')
                        mkdir(dirpath);
                    end
                    savefig(summaryFigure,[dirpath 'IOS_ ' hemisphere '_' dataType '_' variable '_SI_GCaMP']);
                end
            end
        end
    end
...
if saveFigs == true
    % statistical diary
    diaryFile = [dirpath 'WhiskingBehavior_SI_GCaMP_Statistics.txt'];
