function [] = IntrinsicSignals_GCaMP_FC_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(dataTypes)
    ...
    for bb = 1:length(hemispheres)
        ...
        for cc = 1:length(variables)
            ...
            sgtitle(['IOS ' hemisphere ' ' dataType ' ' variable ' [GCaMP FC]'])
            for dd = 1:length(behaviors)
                behavior = behaviors{1,dd};
                subplot(2,3,dd);
                ...
                if saveFigs == true
                    ...
                    savefig(summaryFigure,[dirpath 'IntrinsicSignals_GCaMP_FC_ ' hemisphere '_' dataType '_' variable]);
                    set(summaryFigure,'PaperPositionMode','auto');
                    print('-vector','-dpdf','-fillpage',[dirpath 'IntrinsicSignals_GCaMP_FC_ ' hemisphere '_' dataType '_' variable])
                end
            end
        end
    end
...
if saveFigs == true
    % statistical diary
    diaryFile = [dirpath 'IntrinsicSignals_GCaMP_FC_Statistics.txt'];
