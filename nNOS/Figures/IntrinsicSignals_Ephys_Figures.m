function [] = IntrinsicSignals_Ephys_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(dataTypes)
    ...
    for bb = 1:length(hemispheres)
        ...
        for cc = 1:length(variables)
            ...
            sgtitle(['IOS ' hemisphere ' ' dataType ' ' variable ' [Ephys]'])
            for dd = 1:length(behaviors)
                behavior = behaviors{1,dd};
                subplot(2,3,dd);
                xInds = ones(1,length(data.Naive.(hemisphere).(dataType).(behavior).(variable)));
                s1 = scatter(xInds*1,data.Naive.(hemisphere).(dataType).(behavior).(variable),75,'MarkerEdgeColor','k','MarkerFaceColor',colors('sapphire'),'jitter','off','jitterAmount',0.25);
                hold on;
                e1 = errorbar(1,data.Naive.(hemisphere).(dataType).(behavior).(['mean_' variable]),data.Naive.(hemisphere).(dataType).(behavior).(['std_' variable]),'d','MarkerEdgeColor','k','MarkerFaceColor','k');
                e1.Color = 'black';
                e1.MarkerSize = 10;
                e1.CapSize = 10;
                xInds = ones(1,length(data.Blank_SAP.(hemisphere).(dataType).(behavior).(variable)));
                s2 = scatter(xInds*2,data.Blank_SAP.(hemisphere).(dataType).(behavior).(variable),75,'MarkerEdgeColor','k','MarkerFaceColor',colors('north texas green'),'jitter','off','jitterAmount',0.25);
                e2 = errorbar(2,data.Blank_SAP.(hemisphere).(dataType).(behavior).(['mean_' variable]),data.Blank_SAP.(hemisphere).(dataType).(behavior).(['std_' variable]),'d','MarkerEdgeColor','k','MarkerFaceColor','k');
                e2.Color = 'black';
                e2.MarkerSize = 10;
                e2.CapSize = 10;
                xInds = ones(1,length(data.SSP_SAP.(hemisphere).(dataType).(behavior).(variable)));
                s3 = scatter(xInds*3,data.SSP_SAP.(hemisphere).(dataType).(behavior).(variable),75,'MarkerEdgeColor','k','MarkerFaceColor',colors('electric purple'),'jitter','off','jitterAmount',0.25);
                e3 = errorbar(3,data.SSP_SAP.(hemisphere).(dataType).(behavior).(['mean_' variable]),data.SSP_SAP.(hemisphere).(dataType).(behavior).(['std_' variable]),'d','MarkerEdgeColor','k','MarkerFaceColor','k');
                e3.Color = 'black';
                e3.MarkerSize = 10;
                e3.CapSize = 10;
                title(behavior)
...
if saveFigs == true
    % statistical diary
    diaryFile = [dirpath 'IntrinsicSignals_Ephys_Statistics.txt'];
