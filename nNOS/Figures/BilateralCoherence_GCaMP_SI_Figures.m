function [] = BilateralCoherence_GCaMP_SI_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
        ...
        for cc = 1:length(dataTypes)
            ...
            for dd = 1:length(behaviors)
                ...
                if isempty(Results_BilatCoher_GCaMP.(group).(animalID).(dataType).(behavior).C) == false
                    data.(group).(dataType).(behavior).C = cat(1,data.(group).(dataType).(behavior).C,Results_BilatCoher_GCaMP.(group).(animalID).(dataType).(behavior).C.^2');
...
for aa = 1:length(dataTypes)
    ...
    sgtitle([dataType ' bilateral coherence [GCaMP SI]'])
    for bb = 1:length(behaviors)
        behavior = behaviors{1,bb};
        subplot(2,3,bb);
...
for aa = 1:length(dataTypes)
    dataType = dataTypes{1,aa};
    summaryFigure = figure;
    sgtitle([dataType ' peak coherence [GCaMP SI]'])
...
if saveFigs == true
    % statistical diary
    diaryFile = [dirpath 'BilateralCoherence_GCaMP_SI_Statistics.txt'];
