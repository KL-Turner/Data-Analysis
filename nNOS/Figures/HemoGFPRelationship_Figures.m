function [] = HemoGFPRelationship_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for ee = 1:length(groups)
    ...
    data.(group).stdErrDiameter = std(data.(group).diameter,0,1)./sqrt(size(data.(group).diameter,1));
...
% save figure(s)
if saveFigs == true
    dirpath = [rootFolder delim 'Summary Figures' delim 'Baseline Shift 2P' delim];
