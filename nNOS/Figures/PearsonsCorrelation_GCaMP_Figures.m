function [] = PearsonsCorrelation_GCaMP_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for qq = 1:length(dataTypes)
    ...
    sgtitle([dataType ' Pearsons Corr [GCaMP]'])
    ...
    % save figure(s)
    if saveFigs == true
        dirpath = [rootFolder delim 'Summary Figures' delim 'Pearsons Corr GCaMP' delim];
        if ~exist(dirpath,'dir')
            mkdir(dirpath);
        end
        savefig(summaryFigure,[dirpath 'PearsonsCorr_ ' dataType '_GCaMP']);
    end
end
