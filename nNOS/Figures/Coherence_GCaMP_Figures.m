function [] = Coherence_GCaMP_Figures(rootFolder,~,delim)
...
resultsStruct = 'Results_Coher_GCaMP';
load(resultsStruct);
cd(rootFolder)
groups = {'SSP_SAP','Blank_SAP'};
behaviors = {'Rest','NREM','REM','Alert','Asleep','All'};
dataTypes = {'CBV_HbT','GCaMP7s'};
variables = {'C','f'};
% extract analysis results
for aa = 1:length(groups)
    group = groups{1,aa};
    animalIDs = fieldnames(Results_Coher_GCaMP.(group));
    for bb = 1:length(animalIDs)
        ...
        for cc = 1:length(behaviors)
            ...
            for dd = 1:length(dataTypes)
                ...
                data.(group).(behavior).(dataType).C = cat(1,data.(group).(behavior).(dataType).C,Results_Coher_GCaMP.(group).(animalID).(behavior).(dataType).C.^2');
                data.(group).(behavior).(dataType).f = cat(1,data.(group).(behavior).(dataType).f,Results_Coher_GCaMP.(group).(animalID).(behavior).(dataType).f);
            end
        end
...
for aa = 1:length(dataTypes)
    ...
    for bb = 1:length(behaviors)
        behavior = behaviors{1,bb};
        subplot(2,3,bb);
        p1 = semilogx(data.Blank_SAP.(behavior).(dataType).mean_f,data.Blank_SAP.(behavior).(dataType).mean_C,'color',colors('north texas green'),'LineWidth',2);
        hold on
        ...
        semilogx(data.Blank_SAP.(behavior).(dataType).mean_f,data.Blank_SAP.(behavior).(dataType).mean_C - data.Blank_SAP.(behavior).(dataType).stdErr_C,'color',colors('north texas green'),'LineWidth',0.5);
        p2 = semilogx(data.SSP_SAP.(behavior).(dataType).mean_f,data.SSP_SAP.(behavior).(dataType).mean_C,'color',colors('electric purple'),'LineWidth',2);
