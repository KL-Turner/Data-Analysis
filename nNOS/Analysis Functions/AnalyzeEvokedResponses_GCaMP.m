function [Results_Evoked_GCaMP] = AnalyzeEvokedResponses_GCaMP(animalID,group,set,rootFolder,delim,Results_Evoked_GCaMP)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(hemispheres)
    ...
    for bb = 1:length(dataTypes)
        ...
        for cc = 1:length(WhiskCriteriaNames)
            ...
            [finalWhiskData,~,~,finalWhiskFileEventTimes] = RemoveInvalidData_IOS(whiskData,whiskFileIDs,whiskDurations,whiskEventTimes,ManualDecisions);
        ...
        for gg = 1:length(stimCriteriaNames)
            ...
            % keep only the data that occurs within the manually-approved awake regions
            [finalStimHbTData,~,~,finalStimFileEventTimes] = RemoveInvalidData_IOS(stimData,stimFileIDs,stimDurations,stimEventTimes,ManualDecisions);
