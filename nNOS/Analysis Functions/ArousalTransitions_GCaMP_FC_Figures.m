function [] = ArousalTransitions_GCaMP_FC_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(groups)
    ...
    for bb = 1:length(hemispheres)
        ...
        for cc = 1:length(hemoDataTypes)
            ...
            sgtitle([strrep(group,'_',' ') ' ' hemisphere ' ' hemoDataType ' arousal-state transitions [GCaMP FC]'])
            %% Awake to NREM
            ax1 = subplot(6,2,1);
            % HbT and EMG
            ...
            if saveFigs == true
                ...
                savefig(summaryFigure,[dirpath 'ArousalTransitions_GCaMP_FC_' group '_' hemisphere '_' hemoDataType]);
            end
        end
    end
end
...
for aa = 1:length(hemispheres)
    ...
    for bb = 1:length(hemoDataTypes)
        ...
        summaryFigure = figure;
        sgtitle([hemisphere ' ' hemoDataType ' arousal-state transitions [GCaMP FC]'])
        %% Awake to NREM
        ax1 = subplot(2,2,1);
        % HbT and EMG
        ...
        % save figure(s)
        if saveFigs == true
            savefig(summaryFigure,[dirpath 'ArousalTransitions_GCaMP_FC_ ' hemisphere '_' hemoDataType]);
        end
    end
end