function [Results_ModelAccuracy_Ephys] = AnalyzeModelAccuracy_Ephys(animalID,group,set,rootFolder,delim,Results_ModelAccuracy_Ephys)
%----------------------------------------------------------------------------------------------------------
...
Results_ModelAccuracy_Ephys.(group).(animalID).ModelAccuracy.oobErr = oobError(RF_MDL,'Mode','Ensemble')*100;
...
for aa = 1:iterations
    ...
    Results_ModelAccuracy_Ephys.(group).(animalID).ModelAccuracy.shuff_oobErr(aa,1) = oobError(shuffRF_MDL,'Mode','Ensemble')*100;
end
% save results
