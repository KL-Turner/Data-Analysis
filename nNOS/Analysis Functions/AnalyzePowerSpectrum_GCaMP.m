function [Results_PowerSpecGCaMP] = AnalyzePowerSpectrum_GCaMP(animalID,group,rootFolder,delim,Results_PowerSpecGCaMP)
...
for aa = 1:length(dataTypes)
    ...
    % save results
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).LH.S = LH_rest_S;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).LH.f = LH_rest_f;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).LH.sErr = LH_rest_sErr;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).RH.S = RH_rest_S;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).RH.f = RH_rest_f;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).RH.sErr = RH_rest_sErr;
    % calculate the power spectra of the desired signals
    ...
    % save results
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).LH.fS = fLH_rest_S;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).LH.ff = fLH_rest_f;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).LH.fsErr = fLH_rest_sErr;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).RH.fS = fRH_rest_S;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).RH.ff = fRH_rest_f;
    Results_PowerSpecGCaMP.(animalID).Rest.(dataType).RH.fsErr = fRH_rest_sErr;
    ...
    if isempty(LH_AlertData) == false
        ...
        % save results
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.S = LH_alert_S;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.f = LH_alert_f;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.sErr = LH_alert_sErr;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.S = RH_alert_S;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.f = RH_alert_f;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.sErr = RH_alert_sErr;
        % calculate the power spectra of the desired signals
        ...
        % save results
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.fS = fLH_alert_S;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.ff = fLH_alert_f;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.fsErr = fLH_alert_sErr;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.fS = fRH_alert_S;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.ff = fRH_alert_f;
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.fsErr = fRH_alert_sErr;
    else
        % save results
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.S = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.f = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.sErr = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.S = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.f = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.sErr = [];
        % save results
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.fS = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.ff = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).LH.fsErr = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.fS = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.ff = [];
        Results_PowerSpecGCaMP.(animalID).Alert.(dataType).RH.fsErr = [];
    end
    ...
    if isempty(LH_AsleepData) == false
        ...
        % save results
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.S = LH_asleep_S;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.f = LH_asleep_f;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.sErr = LH_asleep_sErr;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.S = RH_asleep_S;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.f = RH_asleep_f;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.sErr = RH_asleep_sErr;
        % calculate the power spectra of the desired signals
        params.tapers = [10,19]; % Tapers [n, 2n - 1]
        ...
        % save results
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.fS = fLH_asleep_S;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.ff = fLH_asleep_f;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.fsErr = fLH_asleep_sErr;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.fS = fRH_asleep_S;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.ff = fRH_asleep_f;
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.fsErr = fRH_asleep_sErr;
    else
        % save results
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.S = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.f = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.sErr = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.S = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.f = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.sErr = [];
        % save results
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.fS = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.ff = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).LH.fsErr = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.fS = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.ff = [];
        Results_PowerSpecGCaMP.(animalID).Asleep.(dataType).RH.fsErr = [];
    end
    ...
    % save results
    Results_PowerSpecGCaMP.(animalID).All.(dataType).LH.S = LH_all_S;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).LH.f = LH_all_f;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).LH.sErr = LH_all_sErr;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).RH.S = RH_all_S;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).RH.f = RH_all_f;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).RH.sErr = RH_all_sErr;
    % calculate the power spectra of the desired signals
    ...
    % save results
    Results_PowerSpecGCaMP.(animalID).All.(dataType).LH.fS = fLH_all_S;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).LH.ff = fLH_all_f;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).LH.fsErr = fLH_all_sErr;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).RH.fS = fRH_all_S;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).RH.ff = fRH_all_f;
    Results_PowerSpecGCaMP.(animalID).All.(dataType).RH.fsErr = fRH_all_sErr;
    ...
    % save results
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).LH.S = LH_nrem_S;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).LH.f = LH_nrem_f;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).LH.sErr = LH_nrem_sErr;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).RH.S = RH_nrem_S;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).RH.f = RH_nrem_f;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).RH.sErr = RH_nrem_sErr;
    % calculate the power spectra of the desired signals
    ...
    % save results
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).LH.fS = fLH_nrem_S;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).LH.ff = fLH_nrem_f;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).LH.fsErr = fLH_nrem_sErr;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).RH.fS = fRH_nrem_S;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).RH.ff = fRH_nrem_f;
    Results_PowerSpecGCaMP.(animalID).NREM.(dataType).RH.fsErr = fRH_nrem_sErr;
    ...
    % save results
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).LH.S = LH_rem_S;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).LH.f = LH_rem_f;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).LH.sErr = LH_rem_sErr;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).RH.S = RH_rem_S;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).RH.f = RH_rem_f;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).RH.sErr = RH_rem_sErr;
    % calculate the power spectra of the desired signals
    [fLH_rem_S,fLH_rem_f,fLH_rem_sErr] = mtspectrumc(fLH_rem,params);
    [fRH_rem_S,fRH_rem_f,fRH_rem_sErr] = mtspectrumc(fRH_rem,params);
    % save results
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).LH.fS = fLH_rem_S;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).LH.ff = fLH_rem_f;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).LH.fsErr = fLH_rem_sErr;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).RH.fS = fRH_rem_S;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).RH.ff = fRH_rem_f;
    Results_PowerSpecGCaMP.(animalID).REM.(dataType).RH.fsErr = fRH_rem_sErr;
end
% save data
cd([rootFolder delim 'Results_Turner'])
save('Results_PowerSpecGCaMP.mat','Results_PowerSpecGCaMP')
