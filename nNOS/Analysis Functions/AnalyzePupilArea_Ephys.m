function [Results_BehavData] = AnalyzePupilArea_Ephys(animalID,group,set,rootFolder,delim,Results_PupilArea_Ephys)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(dataTypes)
    ...
    % keep only the data that occurs within the manually-approved awake regions
    [LH_finalStimData,LH_finalStimFileIDs,~,~] = RemoveInvalidData_JNeurosci2022(LH_stimData,LH_stimFileIDs,LH_stimDurations,LH_stimEventTimes,ManualDecisions);
    [RH_finalStimData,RH_finalStimFileIDs,~,~] = RemoveInvalidData_JNeurosci2022(RH_stimData,RH_stimFileIDs,RH_stimDurations,RH_stimEventTimes,ManualDecisions);
