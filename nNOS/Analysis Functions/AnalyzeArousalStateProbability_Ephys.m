function [Results_ArousalStateProb_Ephys] = AnalyzeArousalStateProbability_Ephys(animalID,group,set,rootFolder,delim,Results_ArousalStateProb_Ephys)
%----------------------------------------------------------------------------------------------------------
...
[puffLogical] = FilterEvents_IOS(RestData.HbT.LH,RestPuffCriteria);
...
% save results
for f = 1:length(bins)
    Results_ArousalStateProb_Ephys.(group).(animalID).SleepProbability.(bins{1,f}).awakeLogical = data.(bins{1,f}).awakeLogical;
end
...
for a = 1:size(data.uniqueDays,1)
    Results_ArousalStateProb_Ephys.(group).(animalID).Hypnogram.(data.uniqueDays{a,1}) = data.(data.uniqueDays{a,1});
end
