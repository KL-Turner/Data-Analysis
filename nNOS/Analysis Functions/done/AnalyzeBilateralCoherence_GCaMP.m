function [Results_BilatCoher_GCaMP] = AnalyzeBilateralCoherence_GCaMP(animalID,group,set,rootFolder,delim,Results_BilatCoher_GCaMP)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(dataTypes)
    ...
    %% Alert
    clear LH_AlertData RH_AlertData LH_ProcAlertData RH_ProcAlertData fLH_AlertData fRH_AlertData fLH_ProcAlertData fRH_ProcAlertData scoringLabels
    zz = 1;
    ...
    %% Asleep
    clear LH_AsleepData RH_AsleepData LH_ProcAsleepData RH_ProcAsleepData fLH_AsleepData fRH_AsleepData fLH_ProcAsleepData fRH_ProcAsleepData scoringLabels
    zz = 1;
    for bb = 1:size(procDataFileIDs,1)
        procDataFileID = procDataFileIDs(bb,:);
        [~,~,allDataFileID] = GetFileInfo_IOS(procDataFileID);
    ...
    %% All
    clear LH_AllData RH_AllData LH_ProcAllData RH_ProcAllData fLH_AllData fRH_AllData fLH_ProcAllData fRH_ProcAllData
    zz = 1;
    for bb = 1:size(procDataFileIDs,1)
        procDataFileID = procDataFileIDs(bb,:);
        load(procDataFileID,'-mat')
    ...
    %% NREM
    clear LH_nremData RH_nremData fLH_nremData fRH_nremData LH_nrem RH_nrem fLH_nrem fRH_nrem
    ...
    %% REM
    clear LH_remData RH_remData fLH_remData fRH_remData LH_rem RH_rem fLH_rem fRH_rem
    % pull data from SleepData.mat structure
    ...
    % save results
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).C = C_rem;
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).f = f_rem;
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).confC = confC_rem;
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).cErr = cErr_rem;
    % calculate the coherence between desired signals
    [fC_rem,~,~,~,~,ff_rem,fconfC_rem,~,fcErr_rem] = coherencyc(fLH_rem,fRH_rem,params);
    % save results
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).fC = fC_rem;
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).ff = ff_rem;
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).fconfC = fconfC_rem;
    Results_BilatCoher_GCaMP.(group).(animalID).REM.(dataType).fcErr = fcErr_rem;
end
% save data
cd([rootFolder delim 'Results_Turner'])
save('Results_BilatCoherGCaMP.mat','Results_BilatCoher_GCaMP')
