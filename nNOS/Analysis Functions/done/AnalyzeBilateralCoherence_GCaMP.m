function [Results_BilatCoherGCaMP] = AnalyzeBilateralCoherence_GCaMP(animalID,group,rootFolder,delim,Results_BilatCoherGCaMP)
...
for aa = 1:length(dataTypes)
    ...
    % save results
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).C = C_RestData;
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).f = f_RestData;
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).confC = confC_RestData;
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).cErr = cErr_RestData;
    % calculate the coherence between desired signals
    [fC_RestData,~,~,~,~,ff_RestData,fconfC_RestData,~,fcErr_RestData] = coherencyc(fLH_restData,fRH_restData,params);
    % save results
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).fC = fC_RestData;
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).ff = ff_RestData;
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).fconfC = fconfC_RestData;
    Results_BilatCoherGCaMP.(animalID).Rest.(dataType).fcErr = fcErr_RestData;
    ...
    if isempty(LH_AlertData) == false
        ...
        % save results
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).C = C_AlertData;
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).f = f_AlertData;
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).confC = confC_AlertData;
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).cErr = cErr_AlertData;
        % calculate the coherence between desired signals
        [fC_AlertData,~,~,~,~,ff_AlertData,fconfC_AlertData,~,fcErr_AlertData] = coherencyc(fLH_alertData,fRH_alertData,params);
        % save results
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).fC = fC_AlertData;
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).ff = ff_AlertData;
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).fconfC = fconfC_AlertData;
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).fcErr = fcErr_AlertData;
    else
        % save results
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).C = [];
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).f = [];
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).confC = [];
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).cErr = [];
        % save results
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).fC = [];
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).ff = [];
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).fconfC = [];
        Results_BilatCoherGCaMP.(animalID).Alert.(dataType).fcErr = [];
    end
    ...
    if isempty(LH_AsleepData) == false
        ...
        % save results
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).C = C_AsleepData;
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).f = f_AsleepData;
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).confC = confC_AsleepData;
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).cErr = cErr_AsleepData;
        % calculate the coherence between desired signals
        [fC_AsleepData,~,~,~,~,ff_AsleepData,fconfC_AsleepData,~,fcErr_AsleepData] = coherencyc(fLH_asleepData,fRH_asleepData,params);
        % save results
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).fC = fC_AsleepData;
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).ff = ff_AsleepData;
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).fconfC = fconfC_AsleepData;
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).fcErr = fcErr_AsleepData;
    else
        % save results
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).C = [];
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).f = [];
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).confC = [];
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).cErr = [];
        % save results
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).fC = [];
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).ff = [];
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).fconfC = [];
        Results_BilatCoherGCaMP.(animalID).Asleep.(dataType).fcErr = [];
    end
    ...
    % save results
    Results_BilatCoherGCaMP.(animalID).All.(dataType).C = C_AllData;
    Results_BilatCoherGCaMP.(animalID).All.(dataType).f = f_AllData;
    Results_BilatCoherGCaMP.(animalID).All.(dataType).confC = confC_AllData;
    Results_BilatCoherGCaMP.(animalID).All.(dataType).cErr = cErr_AllData;
    % calculate the coherence between desired signals
    [fC_AllData,~,~,~,~,ff_AllData,fconfC_AllData,~,fcErr_AllData] = coherencyc(fLH_allData,fRH_allData,params);
    % save results
    Results_BilatCoherGCaMP.(animalID).All.(dataType).fC = fC_AllData;
    Results_BilatCoherGCaMP.(animalID).All.(dataType).ff = ff_AllData;
    Results_BilatCoherGCaMP.(animalID).All.(dataType).fconfC = fconfC_AllData;
    Results_BilatCoherGCaMP.(animalID).All.(dataType).fcErr = fcErr_AllData;
    %% analyze bilateral coherence during periods of NREM asleep
    clear LH_nremData RH_nremData fLH_nremData fRH_nremData
    clear LH_nrem RH_nrem fLH_nrem fRH_nrem
    ...
    % save results
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).C = C_nrem;
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).f = f_nrem;
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).confC = confC_nrem;
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).cErr = cErr_nrem;
    % calculate the coherence between desired signals
    [fC_nrem,~,~,~,~,ff_nrem,fconfC_nrem,~,fcErr_nrem] = coherencyc(fLH_nrem,fRH_nrem,params);
    % save results
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).fC = fC_nrem;
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).ff = ff_nrem;
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).fconfC = fconfC_nrem;
    Results_BilatCoherGCaMP.(animalID).NREM.(dataType).fcErr = fcErr_nrem;
    %% analyze bilateral coherence during periods of REM asleep
    clear LH_remData RH_remData fLH_remData fRH_remData
    clear LH_rem RH_rem fLH_rem fRH_rem
    % pull data from SleepData.mat structure
    ...
    % save results
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).C = C_rem;
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).f = f_rem;
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).confC = confC_rem;
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).cErr = cErr_rem;
    % calculate the coherence between desired signals
    [fC_rem,~,~,~,~,ff_rem,fconfC_rem,~,fcErr_rem] = coherencyc(fLH_rem,fRH_rem,params);
    % save results
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).fC = fC_rem;
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).ff = ff_rem;
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).fconfC = fconfC_rem;
    Results_BilatCoherGCaMP.(animalID).REM.(dataType).fcErr = fcErr_rem;
end
% save data
cd([rootFolder delim 'Results_Turner'])
save('Results_BilatCoherGCaMP.mat','Results_BilatCoherGCaMP')
