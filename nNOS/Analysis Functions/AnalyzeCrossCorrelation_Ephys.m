...
for aa = 1:length(hemispheres)
    ...
    for qq = 1:length(dataTypes)
        ...
        [restFinalRestHbTData,~,~,~] = RemoveInvalidData_IOS(restingHbTData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
        ...
        for cc = 1:size(procDataFileIDs,1)
            ...
            if sum(strcmp(scoringLabels,'Not Sleep')) > 144 % 12 minutes of asleep
                ...
                if isempty(puffs) == true
                    % alertHbT{zz,1} = filtfilt(sos,g,detrend(ProcData.data.HbT.(hemisphere){bb,1}),'constant');
                    % alertNeural{zz,1} = filtfilt(sos,g,detrend(ProcData.data.cortical_LH.(dataType){bb,1}),'constant');
                    alertHbT{zz,1} = detrend(ProcData.data.HbT.(hemisphere),'constant');
                    alertNeural{zz,1} = detrend(ProcData.data.(neuralDataType).(dataType),'constant');
                    zz = zz + 1;
                end
            end
        end
        ...
        for cc = 1:size(procDataFileIDs,1)
            ...
            if sum(strcmp(scoringLabels,'Not Sleep')) < 36 % 12 minutes of asleep
                ...
                if isempty(puffs) == true
                    ...
                    % asleepHbT{zz,1} = filtfilt(sos,g,detrend(ProcData.data.HbT.(hemisphere){bb,1)},'constant');
                    % asleepNeural{zz,1} = filtfilt(sos,g,detrend(ProcData.data.cortical_LH.(dataType){bb,1}),'constant');
                    asleepHbT{zz,1} = detrend(ProcData.data.HbT.(hemisphere),'constant');
                    asleepNeural{zz,1} = detrend(ProcData.data.(neuralDataType).(dataType),'constant');
                    zz = zz + 1;
                end
            end
        end
        ...
        for cc = 1:size(procDataFileIDs,1)
            ...
            % don't include trials with stimulation
            if isempty(puffs) == true
                % allHbT{zz,1} = filtfilt(sos,g,detrend(ProcData.data.HbT.(hemisphere){bb,1}),'constant');
                % allNeural{zz,1} = filtfilt(sos,gdetrend(ProcData.data.cortical_LH.(dataType){bb,1}),'constant');
                allHbT{zz,1} = detrend(ProcData.data.HbT.(hemisphere),'constant');
                allNeural{zz,1} = detrend(ProcData.data.(neuralDataType).(dataType),'constant');
                zz = zz + 1;
            end
        end
