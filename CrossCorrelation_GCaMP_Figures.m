...
for aa = 1:length(hemispheres)
    ...
    for bb = 1:length(dataTypes)
        ...
        for cc = 1:length(behaviors)
            ...
            hold on
            plot(data.Naive.(hemisphere).(dataType).(behavior).mean_lags,data.Naive.(hemisphere).(dataType).(behavior).mean_xcVals + data.Naive.(hemisphere).(dataType).(behavior).stdErr_xcVals,'color',colors('sapphire'),'LineWidth',0.5);
            plot(data.Naive.(hemisphere).(dataType).(behavior).mean_lags,data.Naive.(hemisphere).(dataType).(behavior).mean_xcVals - data.Naive.(hemisphere).(dataType).(behavior).stdErr_xcVals,'color',colors('sapphire'),'LineWidth',0.5);
            ...
            plot(data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_lags,data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_xcVals + data.Blank_SAP.(hemisphere).(dataType).(behavior).stdErr_xcVals,'color',colors('north texas green'),'LineWidth',0.5);
            plot(data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_lags,data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_xcVals - data.Blank_SAP.(hemisphere).(dataType).(behavior).stdErr_xcVals,'color',colors('north texas green'),'LineWidth',0.5);
            ...
            plot(data.SSP_SAP.(hemisphere).(dataType).(behavior).mean_lags,data.SSP_SAP.(hemisphere).(dataType).(behavior).mean_xcVals + data.SSP_SAP.(hemisphere).(dataType).(behavior).stdErr_xcVals,'color',colors('electric purple'),'LineWidth',0.5);
            plot(data.SSP_SAP.(hemisphere).(dataType).(behavior).mean_lags,data.SSP_SAP.(hemisphere).(dataType).(behavior).mean_xcVals - data.SSP_SAP.(hemisphere).(dataType).(behavior).stdErr_xcVals,'color',colors('electric purple'),'LineWidth',0.5);
            ylabel('Corr Coef')
            xlabel('Lags (s)')
            title(behavior)
        ...
        % save figure(s)
        if saveFigs == true
            dirpath = [rootFolder delim 'Summary Figures' delim 'Cross Corr Ephys' delim];
            if ~exist(dirpath,'dir')
                mkdir(dirpath);
            end
            savefig(summaryFigure,[dirpath 'CrossCorr_' hemisphere '_' dataType '_HbT_Ephys']);
        end
    end
end