...
for b = 1:length(firstsFileOfDay)
    ...
    while strcmp(correctionDecision,'n') == true
        LH_applyCorrection = input(['Apply correction profile to ' strDay ' pixel values? (O/A/B/C): '],'s'); disp(' ')
    ...
    correctionDecision = 'n';
    while strcmp(correctionDecision,'n') == true
        RH_applyCorrection = input(['Apply correction profile to ' strDay ' pixel values? (O/A/B/C): '],'s'); disp(' ')
