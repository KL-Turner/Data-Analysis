...
for f = 1:size(mscanDataFiles, 1)
    ...
    if MScanData.Notes.checklist.processData == false
        ...
        % Trim any additional data points for resample
        trimmedEMG = MScanData.Data.MScan_EMG(1:min(expectedLength, length(MScanData.Data.MScan_EMG)));
        
        % Filter then downsample the Force Sensor waveform to desired frequency
        emgDownSampledSamplingRate = 30;
        emgFilterThreshold = 20;
        emgFilterOrder = 2;
        [z, p, k] = butter(emgFilterOrder, emgFilterThreshold / (MScanData.Notes.MScan_analogSamplingRate / 2), 'low');
        [sos, g] = zp2sos(z, p, k);
