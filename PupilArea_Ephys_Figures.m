function [] = PupilArea_Ephys_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
        ...
        for cc = 1:length(dataTypes)
            ...
            for dd = 1:length(behaviors)
                ...
                if isempty(Results_PupilArea_Ephys.(group).(animalID).(dataType).(behavior).eventMeans) == false
                    data.(group).(dataType).(behavior).data = cat(1,data.(group).(dataType).(behavior).data,mean(Results_PupilArea_Ephys.(group).(animalID).(dataType).(behavior).eventMeans));
...
for aa = 1:length(dataTypes)
    ...
    if saveFigs == true
        ...
        savefig(summaryFigure,[dirpath 'PupilSize_Epyhs_' dataType]);
    end
end