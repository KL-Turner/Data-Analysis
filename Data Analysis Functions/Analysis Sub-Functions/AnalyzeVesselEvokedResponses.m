function [AnalysisResults] = AnalyzeVesselEvokedResponses(animalID,saveFigs,rootFolder,AnalysisResults)
    ...
    for qq = 1:length(whiskCriteriaNames)
        ...
        for aa = 1:length(whiskArterioleIDs)
            ...
            if strcmp(saveFigs,'y') == true
                ...
                hold on
                plot(timeVector,meanWhiskEvokedDiam.(vID) + stdWhiskEvokedDiam.(vID),'color',colors('battleship grey'))
                plot(timeVector,meanWhiskEvokedDiam.(vID) - stdWhiskEvokedDiam.(vID),'color',colors('battleship grey'))
