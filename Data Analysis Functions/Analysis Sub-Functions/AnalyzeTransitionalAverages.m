...
if any(strcmp(animalIDs,animalID))
    ...
    if strcmp(saveFigs,'y') == true
        for e = 1:length(transitions)
            ...
            % behavior
            subplot(3,1,1)
            p1 = plot((1:length(AnalysisResults.(animalID).Transitions.(transition).HbT))/samplingRate,AnalysisResults.(animalID).Transitions.(transition).HbT,'color',colors('rich black'),'LineWidth',2);
            ylabel('\DeltaHbT (%)')
