...
e3.MarkerSize = 10;
e3.CapSize = 10;
s4 = scatter(ones(1,length(procData.NREM.meanArea))*4,procData.NREM.meanArea,75,'MarkerEdgeColor','k','MarkerFaceColor',colorNREM,'jitter','on','jitterAmount',0.25);
e4 = errorbar(4,procData.NREM.mean,procData.NREM.stdev,'d','MarkerEdgeColor','k','MarkerFaceColor','k');
e4.Color = 'black';
e4.MarkerSize = 10;
e4.CapSize = 10;
s5 = scatter(ones(1,length(procData.REM.meanArea))*5,procData.REM.meanArea,75,'MarkerEdgeColor','k','MarkerFaceColor',colorREM,'jitter','on','jitterAmount',0.25);
e5 = errorbar(5,procData.REM.mean,procData.REM.stdev,'d','MarkerEdgeColor','k','MarkerFaceColor','k');
e5.Color = 'black';
e5.MarkerSize = 10;
e5.CapSize = 10;
...
ax2 = subplot(1,2,2);
[xCurve1,yCurve1] = SmoothHistogramBinsFit(procData.Rest.indArea,11,'kernel');
[xCurve2,yCurve2] = SmoothHistogramBinsFit(procData.Whisk.indArea,11,'kernel');
[xCurve3,yCurve3] = SmoothHistogramBinsFit(procData.Stim.indArea,11,'kernel');
[xCurve4,yCurve4] = SmoothHistogramBinsFit(procData.NREM.indArea,31,'kernel');
[xCurve5,yCurve5] = SmoothHistogramBinsFit(procData.REM.indArea,11,'kernel');
...
% save figure(s)
if saveFigs == true
    dirpath = [rootFolder delim 'Summary Figures and Structures' delim 'Arousal State Pupil Area' delim];
    if ~exist(dirpath,'dir')
        mkdir(dirpath);
    end
    savefig(summaryFigure,[dirpath 'Arousal_PupilArea']);
    set(summaryFigure,'PaperPositionMode','auto');
    print('-painters','-dpdf','-bestfit',[dirpath 'Arousal_PupilArea'])
end
