function [Results_Evoked] = AnalyzeSagittalSinusGFP_GarborgTBD(animalID,rootFolder,delim,Results_Evoked)
...
puffFrameStack = [];
for qq = 1:length(uniqueDays)
    fileList = {}; vv = 1;
    for uu = 1:size(procDataFileIDs,1)
        if strfind(procDataFileIDs(uu,:),uniqueDays{qq,1}) > 1
            fileList{vv,1} = procDataFileIDs(uu,:);
            vv = vv + 1;
        end
    end
    for aa = 1:size(fileList)
        puffDataFileID = fileList{aa,1};
        load(puffDataFileID)
