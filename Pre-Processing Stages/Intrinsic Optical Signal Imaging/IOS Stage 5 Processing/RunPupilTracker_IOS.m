...
for cc = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData.data,'Pupil') == false
        ...
        try
            patchedPupilAreaA = fillmissing(testPupilAreaA,'movmedian',max(patchLength)*2);
        catch
            patchedPupilAreaA = testPupilAreaA;
        end
        % patch sudden spikes
        diffArea = abs(diff(patchedPupilAreaA));
        ...
        % fill from start:ending edges of rapid pupil fluctuations that weren't NaN
        testPupilAreaB = patchedPupilAreaA;
        ...
        ProcData.data.Pupil.pupilArea = testPupilAreaB;
