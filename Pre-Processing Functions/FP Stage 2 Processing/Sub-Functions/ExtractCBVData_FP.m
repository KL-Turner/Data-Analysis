function [] = ExtractCBVData_FP(ROIs,ROInames,rawDataFileIDs)
...
for a = 1:size(rawDataFileIDs,1)
    ...
    load(rawDataFile)
    [frames] = ReadDalsaBinary_FP(animalID,[fileID '_WindowCam.bin']);
    for b = 1:length(ROInames)
        ...
        meanIntensity = BinToIntensity_FP(mask,frames);
