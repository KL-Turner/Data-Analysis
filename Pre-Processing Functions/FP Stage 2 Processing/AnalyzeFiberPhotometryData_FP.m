function AnalyzeFiberPhotometryData_FP(csvFiles)
    ...
    for aa = 1:length(fileBreaksIndex) + 1
        if aa == 1
            for bb = 1:length(fields)
                FiberData.data.(fields{1,bb}).raw = channelData(1:fileBreaksIndex(1),bb);
                FiberData.data.(fields{1,bb}).zScored = zScoredFiberData(1:fileBreaksIndex(1),bb);
            end
            FiberData.data.sync = syncData(1:fileBreaksIndex(1));
            % photobleaching removed and HbT corrected
            FiberData.data.RH_405.rescaledData = rescaledData(1:fileBreaksIndex(1),1);
            FiberData.data.RH_465.rescaledData = Corrected465.RH(1:fileBreaksIndex(1));
            FiberData.data.RH_560.rescaledData = rescaledData(1:fileBreaksIndex(1),3);
            FiberData.data.LH_405.rescaledData = rescaledData(1:fileBreaksIndex(1),4);
            FiberData.data.LH_465.rescaledData = Corrected465.LH(1:fileBreaksIndex(1));
            FiberData.data.LH_560.rescaledData = rescaledData(1:fileBreaksIndex(1),6);
            % HbT uncorrected
            FiberData.data.RH_465.uncorrected = rescaledData(1:fileBreaksIndex(1),2);
            FiberData.data.LH_465.uncorrected = rescaledData(1:fileBreaksIndex(1),5);
            FiberData.data.RH_465.zScoredUncorrected = uncorrectedzScoredGCaMPData.RH(1:fileBreaksIndex(1));
            FiberData.data.LH_465.zScoredUncorrected = uncorrectedzScoredGCaMPData.LH(1:fileBreaksIndex(1));
            save([FiberData.notes.animalID '_' FiberData.notes.date '_' FiberData.notes.sessionNumber '_Trial'  num2str(aa) '_FiberData.mat'],'FiberData','-v7.3');
        elseif aa == length(fileBreaksIndex) + 1
            for bb = 1:length(fields)
                FiberData.data.(fields{1,bb}).raw = channelData(fileBreaksIndex(aa - 1) + 1:end,bb);
                FiberData.data.(fields{1,bb}).zScored = zScoredFiberData(fileBreaksIndex(aa - 1) + 1:end,bb);
            end
            FiberData.data.sync = syncData(fileBreaksIndex(aa - 1) + 1:end);
            % photobleaching removed and HbT corrected
            FiberData.data.RH_405.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:end,1);
            FiberData.data.RH_465.rescaledData = Corrected465.RH(fileBreaksIndex(aa - 1) + 1:end);
            FiberData.data.RH_560.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:end,3);
            FiberData.data.LH_405.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:end,4);
            FiberData.data.LH_465.rescaledData = Corrected465.LH(fileBreaksIndex(aa - 1) + 1:end);
            FiberData.data.LH_560.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:end,6);
            % HbT uncorrected
            FiberData.data.RH_465.uncorrected = rescaledData(fileBreaksIndex(aa - 1) + 1:end,2);
            FiberData.data.LH_465.uncorrected = rescaledData(fileBreaksIndex(aa - 1) + 1:end,5);
            FiberData.data.RH_465.zScoredUncorrected = uncorrectedzScoredGCaMPData.RH(fileBreaksIndex(aa - 1) + 1:end);
            FiberData.data.LH_465.zScoredUncorrected = uncorrectedzScoredGCaMPData.LH(fileBreaksIndex(aa - 1) + 1:end);
            save([FiberData.notes.animalID '_' FiberData.notes.date '_' FiberData.notes.sessionNumber '_Trial'  num2str(aa) '_FiberData.mat'],'FiberData','-v7.3');
        else
            for bb = 1:length(fields)
                FiberData.data.(fields{1,bb}).raw = channelData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),bb);
                FiberData.data.(fields{1,bb}).zScored = zScoredFiberData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),bb);
            end
            FiberData.data.sync = syncData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa));
            % photobleaching removed and HbT corrected
            FiberData.data.RH_405.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),1);
            FiberData.data.RH_465.rescaledData = Corrected465.RH(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa));
            FiberData.data.RH_560.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),3);
            FiberData.data.LH_405.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),4);
            FiberData.data.LH_465.rescaledData = Corrected465.LH(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa));
            FiberData.data.LH_560.rescaledData = rescaledData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),6);
            % HbT uncorrected
            FiberData.data.RH_465.uncorrected = rescaledData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),2);
            FiberData.data.LH_465.uncorrected = rescaledData(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa),5);
            FiberData.data.RH_465.zScoredUncorrected = uncorrectedzScoredGCaMPData.RH(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa));
            FiberData.data.LH_465.zScoredUncorrected = uncorrectedzScoredGCaMPData.LH(fileBreaksIndex(aa - 1) + 1:fileBreaksIndex(aa));
            save([FiberData.notes.animalID '_' FiberData.notes.date '_' FiberData.notes.sessionNumber '_Trial'  num2str(aa) '_FiberData.mat'],'FiberData','-v7.3');
        end
    end
end
