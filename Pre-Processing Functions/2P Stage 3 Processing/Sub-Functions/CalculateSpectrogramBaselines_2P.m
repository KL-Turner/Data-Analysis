function [RestingBaselines] = CalculateSpectrogramBaselines_2P(animal,neuralDataTypes,trialDuration_sec,specDataFiles,RestingBaselines,baselineType)
...
for a = 1:length(neuralDataTypes)
    ...
    for b = 1:length(restFileList)
        ...
        for c = 1:size(specDataFiles,1)
            [~,~,~,specDataFile,~,~] = GetFileInfo2_2P(specDataFiles(c,:));
    ...
    for f = 1:length(uniqueDays)
        ...
        for field = 1:length(fields)
            if strcmp(fields{field}(7:12), uniqueDays{f})
                stringDay = ConvertDate_2P(uniqueDays{f});
