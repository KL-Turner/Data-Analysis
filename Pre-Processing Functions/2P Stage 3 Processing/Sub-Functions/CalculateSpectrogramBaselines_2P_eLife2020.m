function [RestingBaselines] = CalculateSpectrogramBaselines_2P_eLife2020(animal,neuralDataTypes,trialDuration_sec,specDataFiles,RestingBaselines,baselineType)
...
for a = 1:length(neuralDataTypes)
    ...
    for b = 1:length(restFileList)
        ...
        for c = 1:size(specDataFiles,1)
            [~,~,~,specDataFile,~,~] = GetFileInfo2_2P_eLife2020(specDataFiles(c,:));
    ...
    for d = 1:length(restFileList)
        ...
        strDay = ConvertDate_2P_eLife2020(fileID(1:6));
        S1_data = restS1{d,1};
        S5_data = restS5{d,1};
        binSize1 = 30;
        binSize5 = 5;
    ...
    uniqueDays = GetUniqueDays_2P_eLife2020(RestingBaselines.(baselineType).baselineFileInfo.fileIDs);   
    for f = 1:length(uniqueDays)
        g = 1;
        for field = 1:length(fields)
            if strcmp(fields{field}(7:12), uniqueDays{f})
                stringDay = ConvertDate_2P_eLife2020(uniqueDays{f});
