...
for a = 1:size(mscanDataFiles,1)
    ...
    if MScanData.notes.checklist.processData == false
        ...
        for q = 1:length(neuralTypes)
            ...
            % MUA [300-3000 Hz]
            [MScanData.data.(neuralField).muaPower] = ProcessNeuro_2P(MScanData,expectedLength,'MUA',neuralType);
            % Gamma [30-100 Hz]
            [MScanData.data.(neuralField).gammaBandPower] = ProcessNeuro_2P(MScanData,expectedLength,'Gam',neuralType);
            % Beta [13-30 Hz]
            [MScanData.data.(neuralField).betaBandPower] = ProcessNeuro_2P(MScanData,expectedLength,'Beta',neuralType);
            % Alpha [10-13 Hz]
            [MScanData.data.(neuralField).alphaBandPower] = ProcessNeuro_2P(MScanData,expectedLength,'Alpha',neuralType);
            % Theta [4-10 Hz]
            [MScanData.data.(neuralField).thetaBandPower] = ProcessNeuro_2P(MScanData,expectedLength,'Theta',neuralType);
            % Delta [1-4 Hz]
            [MScanData.data.(neuralField).deltaBandPower] = ProcessNeuro_2P(MScanData,expectedLength,'Delta',neuralType);
        end
