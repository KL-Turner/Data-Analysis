function [RestingBaselines] = CalculatePixelBaselines_FP(procDataFileIDs,RestingBaselines,baselineType)
...
for a = 1:length(restFileList)
    ...
    for b = 1:size(procDataFileIDs,1)
        ...
        [animalID,~,procDataFile] = GetFileInfo_FP(procDataFileID);
        if strcmp(fileID,procDataFile)
            ...
            [frames] = GetCBVFrameSubset_FP(windowCamFileID,imageHeight,imageWidth,frameInds);
            break
        end
    end
