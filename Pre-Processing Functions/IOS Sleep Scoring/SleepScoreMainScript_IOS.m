...
TrainSleepModels_IOS(animalIDs);
ComparePredictionAccuracy_IOS
...
for bb = 1:size(animalIDs,2)
    ...
    for c = 1:length(modelNames)
        ...
        [ScoringResults] = PredictBehaviorEvents_IOS(animalIDs{1,bb},startingDirectory,animalDirectory,modelDataFileIDs,modelName);
        ApplySleepLogical_IOS(startingDirectory,trainingDirectory,animalDirectory,modelName,ScoringResults)
        NREMsleepTime = 30;   % seconds
        REMsleepTime = 60;   % seconds
        [SleepData] = CreateSleepData_IOS(startingDirectory,trainingDirectory,animalDirectory,NREMsleepTime,REMsleepTime,modelName,SleepData);
    end
