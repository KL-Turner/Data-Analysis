...
%         intensityThresh=phat(1)+(newThresh*phat(2));
        testImg(threshImg >= intensityThresh) = 1;
        testImg(threshImg < intensityThresh) = 0;
        testThresh = labeloverlay(roiImage(:,:,1),testImg);
        figure(2);imagesc(testThresh);
        colormap gray
        axis image
        axis off
        ...
        xlabel('Pixel intensities');
        ylabel('Bin Counts');
        legend({'Normalized Bin Counts','MLE fit of data',[num2str(intensityThresh) 'StD ROI threshold']},'Location','northwest');
        xlim([0,256]);
        axis square
    end
end
