...
imageHeight = 128;                                                                                                            
imageWidth = 128;
recordingTime_minutes = input('Time in minutes: ');
trialDuration = recordingTime_minutes*60;
...
RH_cbvRefl_norm = ((RH_cbvRefl - mean(RH_cbvRefl))./mean(RH_cbvRefl)).*100;
RH_gcampRefl_norm = ((RH_gcampRefl - mean(RH_gcampRefl))./mean(RH_gcampRefl)).*100;
%% generate figure
% figure; 
...
% ylabel('Percent (%)')
figure; 
plot((1:length(RH_cbvRefl_norm))/(Fs/2),detrend(RH_cbvRefl_norm,'constant'),'g')
hold on
plot((1:length(RH_gcampRefl_norm))/(Fs/2),detrend(RH_gcampRefl_norm,'constant'),'b')
axis tight
legend('CBV','GCaMP7s')
ylabel('Percent (%)')
xlabel('Time (s)')
...
%%
figure
plot((1:length(RH_cbvRefl_norm))/(Fs/2),smoothdata(detrend(RH_cbvRefl_norm,'constant')),'g')
hold on
plot((1:length(RH_gcampRefl_norm))/(Fs/2),smoothdata(detrend(RH_gcampRefl_norm,'constant')),'b')
axis tight
legend('CBV','GCaMP7s')
ylabel('Percent (%)')
xlabel('Time (s)')
