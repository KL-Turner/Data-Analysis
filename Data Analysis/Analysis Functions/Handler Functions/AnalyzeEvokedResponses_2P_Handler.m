function [] = AnalyzeEvokedResponses_2P_Handler(rootFolder,delim,runFromStart)
%----------------------------------------------------------------------------------------------------------
...
cc = 1;
multiWaitbar('Analyzing evoked responses for 2P',0,'Color','A');
for aa = 1:length(groups)
    folderList = dir([groups{1,aa} delim set]);
    folderList = folderList(~startsWith({folderList.name},'.'));
    ...
    for bb = 1:length(animalIDs)
        if isfield(Results_Evoked_2P.(groups{1,aa}),animalIDs{1,bb}) == false
            [Results_Evoked_2P] = AnalyzeEvokedResponses_2P(animalIDs{1,bb},groups{1,aa},set,rootFolder,delim,Results_Evoked_2P);
        end
