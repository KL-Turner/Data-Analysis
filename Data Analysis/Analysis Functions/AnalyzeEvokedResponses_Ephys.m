function [Results_Evoked_Ephys] = AnalyzeEvokedResponses_Ephys(animalID,group,setName,rootFolder,delim,Results_Evoked_Ephys)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(hemispheres)
    ...
    for bb = 1:length(WhiskCriteriaNames)
        ...
        [finalWhiskHbTData,finalWhiskFileIDs,~,finalWhiskFileEventTimes] = RemoveInvalidData_IOS(whiskHbTData,whiskFileIDs,whiskDurations,whiskEventTimes,ManualDecisions);
        [finalWhiskCortMUAData,~,~,~] = RemoveInvalidData_IOS(whiskCortMUAData,whiskFileIDs,whiskDurations,whiskEventTimes,ManualDecisions);
        [finalWhiskHipMUAData,~,~,~] = RemoveInvalidData_IOS(whiskHipMUAData,whiskFileIDs,whiskDurations,whiskEventTimes,ManualDecisions);
        [finalWhiskCortGamData,~,~,~] = RemoveInvalidData_IOS(whiskCortGamData,whiskFileIDs,whiskDurations,whiskEventTimes,ManualDecisions);
        [finalWhiskHipGamData,~,~,~] = RemoveInvalidData_IOS(whiskHipGamData,whiskFileIDs,whiskDurations,whiskEventTimes,ManualDecisions);
    ...
    for gg = 1:length(stimCriteriaNames)
        ...
        % keep only the data that occurs within the manually-approved awake regions
        [finalStimHbTData,finalStimFileIDs,~,finalStimFileEventTimes] = RemoveInvalidData_IOS(stimHbTData,stimFileIDs,stimDurations,stimEventTimes,ManualDecisions);
        [finalStimCortMUAData,~,~,~] = RemoveInvalidData_IOS(stimCortMUAData,stimFileIDs,stimDurations,stimEventTimes,ManualDecisions);
        [finalStimHipMUAData,~,~,~] = RemoveInvalidData_IOS(stimHipMUAData,stimFileIDs,stimDurations,stimEventTimes,ManualDecisions);
        [finalStimCortGamData,~,~,~] = RemoveInvalidData_IOS(stimCortGamData,stimFileIDs,stimDurations,stimEventTimes,ManualDecisions);
        [finalStimHipGamData,~,~,~] = RemoveInvalidData_IOS(stimHipGamData,stimFileIDs,stimDurations,stimEventTimes,ManualDecisions);
