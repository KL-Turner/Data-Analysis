function [] = StimEvoked_Bilateral_GCaMP(rootFolder,saveFigs,delim)
    ...
    for bb = 1:length(animalIDs)
        ...
        for cc = 1:length(hemispheres)
            ...
            for dd = 1:length(solenoidNames)
                ...
                data.(expGroup).(hemisphere).(solenoidName).dummCheck = 1;
                for ee = 1:length(dataTypes)
                    dataType = dataTypes{1,ee};
                    if isfield(data.(expGroup).(hemisphere).(solenoidName),dataType) == false
                        data.(expGroup).(hemisphere).(solenoidName).(dataType) = [];
                    end
                end
                data.(expGroup).(hemisphere).(solenoidName).HbT = cat(1,data.(expGroup).(hemisphere).(solenoidName).HbT,Results_EvokedGCaMP.(expGroup).(animalID).Stim.(hemisphere).(solenoidName).CBV_HbT.HbT);
                data.(expGroup).(hemisphere).(solenoidName).GCaMP = cat(1,data.(expGroup).(hemisphere).(solenoidName).GCaMP,Results_EvokedGCaMP.(expGroup).(animalID).Stim.(hemisphere).(solenoidName).GCaMP.corticalData);
                data.(expGroup).(hemisphere).(solenoidName).timeVector = cat(1,data.(expGroup).(hemisphere).(solenoidName).timeVector,Results_EvokedGCaMP.(expGroup).(animalID).Stim.(hemisphere).(solenoidName).timeVector);
                %                 data.(expGroup).(solenoidName).(hemisphere).count = cat(1,data.(treatment).(solenoidNames{1,bb}).(hemispheres{1,cc}).count,Results_Evoked.(animalIDs.all{1,aa}).Stim.(hemispheres{1,cc}).(solenoidNames{1,bb}).count);
                %                 data.(expGroup).(solenoidName).(hemisphere).undershoot = cat(1,data.(treatment).(solenoidNames{1,bb}).(hemispheres{1,cc}).undershoot,mean(Results_Evoked.(animalIDs.all{1,aa}).Stim.(hemispheres{1,cc}).(solenoidNames{1,bb}).CBV_HbT.HbT(120:180)));
                %                 data.(expGroup).(solenoidName).(hemisphere).animalID = cat(1,data.(treatment).(solenoidNames{1,bb}).(hemispheres{1,cc}).animalID,animalIDs.all(1,aa));
                %                 data.(expGroup).(solenoidName).(hemisphere).treatment = cat(1,data.(treatment).(solenoidNames{1,bb}).(hemispheres{1,cc}).treatment,{treatment});
...
for ff = 1:length(treatments)
    for gg = 1:length(dataTypes)
        data.(treatments{1,ff}).Contra.adjLH.(dataTypes{1,gg}) = data.(treatments{1,ff}).RPadSol.adjLH.(dataTypes{1,gg});
        data.(treatments{1,ff}).Contra.adjRH.(dataTypes{1,gg}) = data.(treatments{1,ff}).LPadSol.adjRH.(dataTypes{1,gg});
    end
    % hip
...
for ff = 1:length(treatments)
    for gg = 1:length(dataTypes)
        data.(treatments{1,ff}).Ipsi.adjLH.(dataTypes{1,gg}) = data.(treatments{1,ff}).RPadSol.adjRH.(dataTypes{1,gg});
        data.(treatments{1,ff}).Ipsi.adjRH.(dataTypes{1,gg}) = data.(treatments{1,ff}).LPadSol.adjLH.(dataTypes{1,gg});
    end
    % hip
    for hh = 1:length(hipVariables)
...
for ff = 1:length(treatments)
    for gg = 1:length(dataTypes)
        data.(treatments{1,ff}).Auditory.adjLH.(dataTypes{1,gg}) = data.(treatments{1,ff}).AudSol.adjLH.(dataTypes{1,gg});
        data.(treatments{1,ff}).Auditory.adjRH.(dataTypes{1,gg}) = data.(treatments{1,ff}).AudSol.adjRH.(dataTypes{1,gg});
    end
    % hip
    for hh = 1:length(hipVariables)
