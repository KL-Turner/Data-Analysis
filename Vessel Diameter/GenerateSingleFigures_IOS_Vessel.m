function [figHandle] = GenerateSingleFigures_IOS_Vessel(procDataFileID,RestingBaselines,baselineType,saveFigs,imagingType,hemoType)
...
elseif strcmp(imagingType,'single') == true
    ...
    elseif strcmp(hemoType,'HbT') == true
        ...
        % veinous
        veinDiam = (ProcData.data.CBV.Veinous - RestingBaselines.(baselineType).CBV.Veinous.(strDay))./(RestingBaselines.(baselineType).CBV.Veinous.(strDay));
        filtVeinDiam = filtfilt(sos2,g2,veinDiam)*100;
    end
end
...
elseif strcmp(imagingType,'single') == true
    ...
    elseif strcmp(hemoType,'HbT') == true
        ...
        ylabel('\DeltaHbT')
        yyaxis right 
        p6 = plot((1:length(filtVeinDiam))/ProcData.notes.CBVCamSamplingRate,filtVeinDiam,'color',colors_Manuscript2020('rich black'),'LineWidth',1);
        ylabel('Diameter (%)')
        legend([p5,p6,s1,s2,s3,s4,s5],'Barrels','Vein','movement','whisking',',LPad sol','RPad sol','Aud sol')
    end
end
xlim([0,ProcData.notes.trialDuration_sec])
set(gca,'TickLength',[0,0])
