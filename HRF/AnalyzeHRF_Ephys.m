...
%                 kernelFig = figure;
%                 sgtitle([animalID ' ' hemisphere ' ' dataType ' during ' behavior])
%                 p1 = plot(Results_HRF_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).IR_timeVec_long,Results_HRF_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).IR_function_long);
%                 hold on
%                 p2 = plot(Results_HRF_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).IR_gammaTimeVec,Results_HRF_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).IR_gammaFunction);
%                 p3 = plot(Results_HRF_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).FM_gammaTimeVec,Results_HRF_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).FM_gammaFunction);
...
%                 set(gca,'box','off')
%                 % save figures
%                 dirpath = [rootFolder delim animalID delim 'Figures' delim 'HRF Kernels' delim dataType delim behavior delim];
%                 if ~exist(dirpath,'dir')
%                     mkdir(dirpath);
%                 end
%                 savefig(kernelFig,[dirpath animalID '_' hemisphere '_' dataType '_' behavior '_(group)']);
%                 close(kernelFig)
%             end
        end
    end
