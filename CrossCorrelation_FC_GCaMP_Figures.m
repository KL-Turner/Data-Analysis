function [] = CrossCorrelation_FC_GCaMP_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(hemispheres)
    ...
    for bb = 1:length(dataTypes)
        ...
        sgtitle([hemisphere ' ' dataType ' cross correlation with GCaMP [GCaMP FC]'])
        ...
        % save figure(s)
        if saveFigs == true
            dirpath = [rootFolder delim 'Summary Figures' delim 'Cross Corr GCaMP FC' delim];
            if ~exist(dirpath,'dir')
                mkdir(dirpath);
            end
            savefig(summaryFigure,[dirpath 'CrossCorr_GCaMP_FC_' hemisphere '_' dataType]);
        end
    end
end