function [] = NeuralHemoCoherence_GCaMP_FC_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(hemispheres)
    ...
    for bb = 1:length(dataTypes)
        ...
        sgtitle([hemisphere ' ' dataType ' neural-hemo coherence [GCaMP FC]'])
        for cc = 1:length(behaviors)
            behavior = behaviors{1,cc};
            subplot(2,3,cc);
        ...
        if saveFigs == true
            ...
            savefig(summaryFigure,[dirpath 'NeuralHemoCoherence_GCaMP_FC_' hemisphere '_' dataType]);
            set(summaryFigure,'PaperPositionMode','auto');
            print('-vector','-dpdf','-fillpage',[dirpath 'NeuralHemoCoherence_GCaMP_FC_' hemisphere '_' dataType])
        end
    end
end