function [] = PowerSpectrum_GCaMP_FC_Figures(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
            ...
            for dd = 1:length(dataTypes)
                ...
                for ee = 1:length(behaviors)
                    ...
                    if isempty(Results_PowerSpec_GCaMP.(group).(animalID).(hemisphere).(dataType).(behavior).S) == false
                        data.(group).(hemisphere).(dataType).(behavior).S = cat(1,data.(group).(hemisphere).(dataType).(behavior).S,Results_PowerSpec_GCaMP.(group).(animalID).(hemisphere).(dataType).(behavior).S');
                        data.(group).(hemisphere).(dataType).(behavior).f = cat(1,data.(group).(hemisphere).(dataType).(behavior).f,Results_PowerSpec_GCaMP.(group).(animalID).(hemisphere).(dataType).(behavior).f);
...
for aa = 1:length(hemispheres)
    ...
    for bb = 1:length(dataTypes)
        ...
        sgtitle([hemisphere ' ' dataType ' power spectrum [GCaMP FC]'])
        for cc = 1:length(behaviors)
            behavior = behaviors{1,cc};
            subplot(2,3,cc);
            p1 = loglog(data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_f,data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_normS,'color',colors('north texas green'),'LineWidth',2);
            hold on;
            ...
            loglog(data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_f,data.Blank_SAP.(hemisphere).(dataType).(behavior).mean_normS - data.Blank_SAP.(hemisphere).(dataType).(behavior).stdErr_normS,'color',colors('north texas green'),'LineWidth',0.25);
            p2 = loglog(data.SSP_SAP.(hemisphere).(dataType).(behavior).mean_f,data.SSP_SAP.(hemisphere).(dataType).(behavior).mean_normS,'color',colors('electric purple'),'LineWidth',2);
        ...
        if saveFigs == true
            ...
            savefig(summaryFigure,[dirpath 'PowerSpectrum_GCaMP_FC_' dataType]);
        end
    end
end