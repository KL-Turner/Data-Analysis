function [] = Fig_temp_nNOS(rootFolder,saveFigs,delim)
%----------------------------------------------------------------------------------------------------------
...
resultsStruct = 'Results_PreWhitenedPowerSpec_Ephys';
load(resultsStruct);
cd(rootFolder)
% loop parameters
...
for aa = 1:length(groups)
    group = groups{1,aa};
    animalIDs = fieldnames(Results_PreWhitenedPowerSpec_Ephys.(group));
        ...
        for cc = 1:length(hemispheres)
            ...
            for dd = 1:length(dataTypes)
                ...
                for ee = 1:length(behaviors)
                    ...
                    if isempty(Results_PreWhitenedPowerSpec_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).S) == false
                        ephysPowerData.(group).(hemisphere).(dataType).(behavior).S = cat(1,ephysPowerData.(group).(hemisphere).(dataType).(behavior).S,Results_PreWhitenedPowerSpec_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).S');
                        ephysPowerData.(group).(hemisphere).(dataType).(behavior).f = cat(1,ephysPowerData.(group).(hemisphere).(dataType).(behavior).f,Results_PreWhitenedPowerSpec_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).f);
                        freqBand = round(Results_PreWhitenedPowerSpec_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).f,2);
                        frequencyList = unique(freqBand);
                        for qq = 1:length(frequencyList)/2
                            freqIdx = find(freqBand == frequencyList(1,qq));
                            ephysPowerData.(group).(hemisphere).(dataType).(behavior).binS = cat(1,ephysPowerData.(group).(hemisphere).(dataType).(behavior).binS,mean(Results_PreWhitenedPowerSpec_Ephys.(group).(animalID).(hemisphere).(dataType).(behavior).S(freqIdx)));
